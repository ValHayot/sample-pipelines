...
class Boutiques2Pydra:
    def __init__(self, descriptor, *args, input_spec=None):
        ...
        if input_spec is None:
            ...
            invocation = dumps(
                {k: v for k, v in loads(invocation).items() if k in input_spec}
            )
            print(invocation)
        ...
        self.pydra_task = r"""def pydra_task({0}):
            ...
            return [out_f.file_name.encode("utf-8") for out_f in ret.output_files]""".format(
            parameters, desc_dict, list(args)
        )
        ...
        self.create_task = pydra.mark.annotate({"return": {"outfile": ty.Any}})(
            self.create_task
        )
